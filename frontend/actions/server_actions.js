...
export const fetchCurrentUserServers = () => dispatch => {
  return APIUtil.fetchCurrentUserServers().then(
...
export const createServer = serverName => dispatch => {
  return APIUtil.createServer(serverName).then(
...
export const joinServer = serverName => dispatch => {
  return APIUtil.joinServer(serverName).then(
    server => dispatch(receiveServer(server)),
    err => dispatch(receiveServerErrors(err.responseJSON))
  );
};
...
export const leaveServer = id => dispatch => {
  return APIUtil.leaveServer(id).then(
